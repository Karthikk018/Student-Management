...
class Student:
    def __init__(self, root):
        self.root = root
        self.root.title("RUAS Portal")
        self.root.geometry("1350x680+0+0")

        title = Label(self.root, text="Ramaiah Student Management System", bd=10, relief=GROOVE,
                      font=("times new roman", 40, "bold"), bg="cornflowerblue", fg="black")
        ...
        Footer_Frame = Frame(self.root, bg="cornflowerblue")
        ...
        lbl_dateofbirth = Label(Manage_Frame, text="D.O.B", font=("times new roman", 20, "bold"), bg="black",
                                fg="white")
        ...
        combo_gender = ttk.Combobox(Manage_Frame, font=("time new roman", 15, "normal"), state="readonly",
        ...
        addbtn = Button(btn_frame, text="Add", width=10, command=self.add_students).grid(row=0, column=0, padx=3,
                                                                                         pady=10)
        updatebtn = Button(btn_frame, text="Update", width=10, command=self.update_data).grid(row=0, column=1, padx=8,
                                                                                              pady=10)
        deletebtn = Button(btn_frame, text="Delete", width=10, command=self.delete_data).grid(row=0, column=2, padx=8,
                                                                                              pady=10)
        ...
        combo_search = ttk.Combobox(Detail_Frame, textvariable=self.search_by, width=15,
                                    font=("time new roman", 13, "normal"), state="readonly")
    ...
    def add_students(self):
        ...
        if self.rollno_var.get() == "" or self.name_var.get() == "":
            ...
            con = pymysql.connect(host="localhost", user="root", password="rcb12345", database="studentdata")
            cur = con.cursor()
    ...
    def search_data(self):
        con = pymysql.connect(host="localhost", user="root", password="rcb12345", database="studentdata")
        cur = con.cursor()
        cur.execute("select * from student_info where " + str(self.search_by.get()) + " LIKE '%" + str(
            self.search_txt.get()) + "%'")
        rows = cur.fetchall()
        if len(rows) != 0:
            self.student_table.delete(*self.student_table.get_children())
